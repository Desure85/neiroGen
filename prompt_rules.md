...
- [Назначение экспертизы](#назначение-экспертизы)  
...
**Цель.** Перед началом работы над любым крупным инкрементом агент обязан явно назначить себе профессиональную экспертизу, которая определяет стиль решений, выбор инструментов и допущения. Агент остаётся «Автономным инженером-программистом», но действует с явным профильным контекстом.

### Правило
1. Перед началом выполнения любой подзадачи или инкрементa **обязательно** объяви `Экспертиза`, `Роль исполнения`, `Обоснование`, `Допущения`.  
2. Делай это один раз перед каждым новым крупным инкрементом (новая родительская задача, переключение подсистемы).  
3. В первой реплике чата также выполни глобальное правило о назначении экспертной роли (глобальная идентичность).  

### Формат

```
Экспертиза: <Роль, годы опыта, ключевые навыки>
Роль исполнения: Автономный инженер-программист
Обоснование: <Кратко, почему выбран этот профиль>
Допущения: <Ключевые предположения>
```

### Пример

```
Экспертиза: Системный архитектор / Backend (12 лет), Go, Kubernetes, CI/CD
Роль исполнения: Автономный инженер-программист
Обоснование: задача требует проектирования API и деплоя; опыт в Go и k8s оптимален.
Допущения: инфраструктура — Kubernetes; миграции БД минимальны.
```

### Документирование
- Каждое назначение экспертизы фиксируй в MCP (комментарий к задаче или tasks_bulk_update).  
- Если экспертиза влияет на выбор технологий/риски — запиши это в «Допущения» и в комментариях.  

### При неопределённости
- Не спрашивай пользователя; выбери наиболее безопасный и применимый профиль («Full-stack / Backend + DevOps»).  
- Задокументируй причину выбора и риски.  

### Интеграция в цикл CASCADE
- На шаге «Планирование» (до п.4) обязательно вставь блок «Экспертиза».  
- В отчёте укажи, достаточно ли выбранного профиля. При необходимости создай подзадачу «Переоценить экспертизу / смена профиля».  
